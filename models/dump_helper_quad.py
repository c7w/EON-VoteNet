...
def dump_results_quad(end_points, dump_dir, config, inference_switch=False):
    ...
    objectness_scores = end_points['quad_scores'].detach().cpu().numpy() # (B,K,2)
    pred_center = end_points['quad_center'].detach().cpu().numpy() # (B,K,3)
    normal_vector =  end_points['normal_vector'].detach().cpu().numpy()
    pred_size = end_points['quad_size'].detach().cpu().numpy()
