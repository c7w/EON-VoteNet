...
def dump_examples(end_points, dump_dir, config, inference_switch=False, batch_idx=0):
    ''' Dump results.
    ...
    # INPUT
    point_clouds = end_points['point_clouds'].cpu().numpy()
    point_clouds_color = end_points['point_cloud_color'].cpu().numpy()
    batch_size = point_clouds.shape[0]
    ...
    if 'vote_xyz' in end_points:
        vote_xyz = end_points['vote_xyz'].detach().cpu().numpy() # (B,num_seed,3)
        aggregated_vote_xyz = end_points['aggregated_vote_xyz'].detach().cpu().numpy()  # [B, n_proposal, 3]
        aggregated_vote_inds = end_points['aggregated_vote_inds'].detach().cpu().numpy()  # [B, n_proposal]
        vote_label = end_points['vote_label'].cpu().numpy()
        vote_mask = end_points['vote_mask'].cpu().numpy()
    objectness_scores = end_points['objectness_scores'].detach().cpu().numpy() # (B,K,2)
    pred_center = end_points['center'].detach().cpu().numpy() # (B,K,3)
    pred_heading_class = torch.argmax(end_points['heading_scores'], -1) # B,num_proposal
    pred_heading_residual = torch.gather(end_points['heading_residuals'], 2, pred_heading_class.unsqueeze(-1)) # B,num_proposal,1
    ...
    i = 0
    pc = point_clouds[i,:,:3]
    pc_color = point_clouds_color[i,:]
    objectness_prob = softmax(objectness_scores[i,:,:])[:,1] # (K,)

    # Dump various point clouds
    pc_util.write_ply(np.hstack([pc, pc_color]), os.path.join(dump_dir, '%06d_pc.ply'%(idx_beg+i)))
    pc_util.write_ply(seed_xyz[i,:,:], os.path.join(dump_dir, '%06d_seed_pc.ply'%(idx_beg+i)))
    if 'vote_xyz' in end_points:
        v_xyz = vote_xyz[i]  # [1024, 3]
        s_xyz = seed_xyz[i]  # [1024, 3]
        pc_to_save = np.vstack([v_xyz, s_xyz])
        n_p = v_xyz.shape[0]
        edges = [(i, i + n_p) for i in range(n_p)]
        pc_util.write_ply_edge(pc_to_save, edges, os.path.join(dump_dir, '%06d_vgen_pc.ply'%(idx_beg+i)))

        vp_xyz = v_xyz[aggregated_vote_inds.squeeze()]  # [64, 3]
        sp_xyz = s_xyz[aggregated_vote_inds.squeeze()]
        pc_to_save = np.vstack([vp_xyz, sp_xyz])
        n_p = vp_xyz.shape[0]
        edges = [(i, i + n_p) for i in range(n_p)]
        pc_util.write_ply_edge(pc_to_save, edges, os.path.join(dump_dir, '%06d_aggregated_vote_pc.ply' % (idx_beg + i)))

        v_label = vote_label[i]
        v_label_mask = vote_mask[i]
        inds = v_label_mask == 1
        pc_vote = pc[inds, :3]
        gt_vote = v_label[inds, :3] + pc_vote
        pc_to_save = np.vstack([pc_vote, gt_vote])
        n_p = pc_vote.shape[0]
        edges = [(i, i + n_p) for i in range(n_p)]
        pc_util.write_ply_edge(pc_to_save, edges, os.path.join(dump_dir, '%06d_gt_vote_pc.ply' % (idx_beg + i)))

        seed_inds = end_points['seed_inds'].cpu().numpy().squeeze()
        mask_seed = inds[seed_inds]
        masked_v_xyz = v_xyz[mask_seed]
        masked_s_xyz = s_xyz[mask_seed]
        pc_to_save = np.vstack([masked_v_xyz, masked_s_xyz])
        n_p = masked_v_xyz.shape[0]
        edges = [(i, i + n_p) for i in range(n_p)]
        pc_util.write_ply_edge(pc_to_save, edges, os.path.join(dump_dir, '%06d_masked_vgen_pc.ply' % (idx_beg + i)))
