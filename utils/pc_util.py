...
def batch_rotz(rot_radian):
    """
    Args:
        rot_radian: [B, Np] or [B, Np, Nnb]

    Returns:
        rot_mat: [B, Np, 3, 3] or [B, Np, Nnb, 3, 3]
    """
    shape_len = len(rot_radian.shape)
    if shape_len == 2:
        B, Np = rot_radian.shape
        rot_mat = torch.eye(3)[None, None].expand(B, Np, -1, -1).clone().to(rot_radian.device)
    elif shape_len == 3:
        B, Np, Nnb = rot_radian.shape
        rot_mat = torch.eye(3)[None, None, None].expand(B, Np, Nnb, -1, -1).clone().to(rot_radian.device)
    else:
        assert False, 'have not implemented this shape length: {}'.format(shape_len)

    cos, sin = torch.cos(rot_radian), torch.sin(rot_radian)
    rot_mat[..., 0, 0], rot_mat[..., 0, 1], rot_mat[..., 1, 0], rot_mat[..., 1, 1] = cos, -sin, sin, cos
    return rot_mat
...
def angle2class(angle, num_class):
    ...
    class_id = (shifted_angle / angle_per_class)
    if isinstance(class_id, torch.Tensor):
        class_id = class_id.int()
        class_id_f = class_id.float()
    elif isinstance(class_id, np.ndarray):
        class_id = class_id.astype(np.int64)
        class_id_f = class_id.astype(np.float32)
    else:
        assert False, 'not implemented for data type: {}'.format(class_id.__class__)
    residual_angle = shifted_angle - (class_id_f * angle_per_class + angle_per_class / 2)
...
def write_ply_edge(points, edges, savepath):
    """
    Args:
        points: Nx3 array
        edges: list of M 2-tuple
        savepath:
    """
    points = [(points[i, 0], points[i, 1], points[i, 2]) for i in range(points.shape[0])]
    vertex = np.array(points, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
    el_v = PlyElement.describe(vertex, 'vertex')
    edges_array = np.array(edges, dtype=[('vertex1', 'i4'), ('vertex2', 'i4')])
    el_e = PlyElement.describe(edges_array, 'edge')
    PlyData([el_v, el_e], text=True).write(savepath)
...
def write_ply_color_continuous(points, mapping, filename, colormap=pyplot.cm.jet):
    """
    Args:
        points: [N, 3]
        mapping: [N,] ranging [0,1], used to select target color
        filename:
        colormap:
    """
    assert points.shape[0] == mapping.shape[0]
    vertex = []
    N = points.shape[0]
    for i in range(N):
        c = colormap(float(mapping[i]))
        c = [int(x * 255) for x in c]
        vertex.append((points[i, 0], points[i, 1], points[i, 2], c[0], c[1], c[2]))
    vertex = np.array(vertex, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('red', 'u1'), ('green', 'u1'), ('blue', 'u1')])
...
def transform_ply(transform_matrix, mesh_obj, color=(50, 50, 200)):
    """transform a bbox and add color to vertices"""
    faces_bbox = []
    verts_bbox = []
    for f in mesh_obj["face"]:
        faces_bbox.append((np.array(f[0]) + len(verts_bbox),))
    for v in mesh_obj["vertex"]:
        v1 = np.array([v[0], v[1], v[2], 1])
        v1 = np.dot(transform_matrix, v1)[0:3]
        verts_bbox.append(tuple(v1) + color)
    return verts_bbox, faces_bbox

def my_write_bbox(save_path, transform=None, bbox_path=None, red_box=False, box_prerotate=0, color_idx=0, color_map=COLOR_MAP):
    if transform is None:
        transform = np.eye(4)
    if bbox_path is None:
        bbox_path = '{}/bbox_faceny_arrow.ply'.format(BASE_DIR)
    with open(bbox_path, 'rb') as read_file:
        mesh_bbox = PlyData.read(read_file)
    pre_rotate = np.eye(4)
    pre_rotate[:3, :3] = utils.pc_util.rotz(box_prerotate)
    transform = np.dot(transform, pre_rotate)
    verts_bbox, faces_bbox = transform_ply(transform, mesh_bbox, color_map[color_idx])
    if red_box:
        verts_bbox = [(x[0], x[1], x[2], 200, 50, 50) for x in verts_bbox]
